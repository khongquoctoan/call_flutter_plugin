import 'dart:convert';
import 'package:call_flutter_plugin/graph/model/oauth.dart';
...
class GraphModule {
  ...
  static const _URL_OAUTH = "http://auth2.callfluttervn/api/token";

  static const _URL_GRAPH = "http://graph.callfluttervn/";

  Future<Oauth> getAccessToken(
      {required String apiKey, required String apiSecert}) async {
    try {
      ...
      if (data == null ||
          ...
          _isStatusCodeSuccess(data)) {
        print(data);
        return Future.value(
            Oauth(token: "", createAt: "", expired: "", isLongLive: false));
      }
      ...
      return Future.value(Oauth(
          token: token,
          createAt: createAt,
          expired: expired,
          isLongLive: isLongLive));
    } catch (exception) {
  ...
  bool _isStatusCodeSuccess(dynamic data) {
    return ((data['data']['response']['status'] is int) &&
            data['data']['response']['status'] != 1000) ||
        (data['data']['response']['status'] is String) &&
            data['data']['response']['status'] != '1000';
  }
}
