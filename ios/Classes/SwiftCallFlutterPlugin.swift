...
public class SwiftCallFlutterPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    
    private var sipManager: SipManager = SipManager.instance
    static var eventSink: FlutterEventSink?
    
    public static func register(with registrar: FlutterPluginRegistrar) {
        let instance = SwiftCallFlutterPlugin()
        
        let methodChannel = FlutterMethodChannel(name: "flutter_call_flutter_plugin_method_channel", binaryMessenger: registrar.messenger())
        registrar.addMethodCallDelegate(instance, channel: methodChannel)
        
        let eventChannel = FlutterEventChannel(name: "flutter_call_flutter_plugin_event_channel", binaryMessenger: registrar.messenger())
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        SwiftCallFlutterPlugin.eventSink = events
        return nil
    }
    
    public func onCancel(withArguments arguments: Any?) -> FlutterError? {
        SwiftCallFlutterPlugin.eventSink = nil
        return nil
    }
}
